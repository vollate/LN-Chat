...
void Controller::handlePublishRoom(CallData* call_data) {
    ...
    if(call_data->m_storage->addRoom(call_data->m_publish_room_request.name(), call_data->m_publish_room_request.password())) {
        auto [lock, room] = call_data->m_storage->getChatRoom(call_data->m_publish_room_request.name()).value();
        room->users.push_back(call_data->m_storage->getClient(call_data->m_publish_room_request.clientid()).value());
...
void Controller::handleGetRoomPeers(CallData* call_data) {
    ...
    if(auto cur_client = call_data->m_storage->getClient(call_data->m_get_room_peers_request.clientid()); !cur_client) {
        ...
        auto& [lock, room] = room_opt.value();
        if(auto block_op = call_data->m_storage->get(room->blacklist, cur_client.value())) {
            call_data->m_get_room_peers_reply.set_success(false);
            ...
            call_data->m_storage->insertOnNotExist(room->users, cur_client.value());
